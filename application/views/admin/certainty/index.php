                    ...
                    <tbody>
                      ...
                      <?php foreach ($certainty1 as $c) : ?>
                        <tr>
                          ...
                          <!-- <td><?= $c['skor'] ?></td> -->
                          <td>
                            <a href="<?= base_url('Admin/certainty/update/') . $c['id'] ?>" id-certainty1="<?= $c['id'] ?>" onclick="show_data(this)" role="button" class="badge badge-sm badge-info badge-pill" data-toggle="modal" data-target="#editModal">Edit</a>
                            <a href="<?= base_url('Admin/certainty/delete/') . $c['id'] ?>" class="badge badge-sm badge-danger badge-pill" onclick="return confirm('apakah anda yakin?');">Hapus</a>
                          </td>
                        </tr>
                      <?php endforeach; ?>
                    </tbody>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="<?= base_url('admin/certainty/update') ?>" method="post">
        <div class="modal-body">
          <input type="hidden" id="id_certainty1" name="id_certainty1">
          <div class="form-group">
            <label>Nama Penyakit</label>
...
<script type="text/javascript">
  ...
  const show_data = async (target) => {
    const id = target.getAttribute('id-certainty1')

    const result = await get_data(id).then(res => res)

    console.log(result)

    document.getElementById('id_certainty1').value = id;
